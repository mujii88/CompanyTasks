...
router.get('/', authenticateToken, async (req, res) => {
  try {
    ...
    if (usingFileDatabase()) {
      ...
      tasks = await fileDatabase.findTasks(query);
      
      // Populate user data manually for file database
      for (let task of tasks) {
        const assignee = await fileDatabase.findUserById(task.assignee);
        const assignedBy = await fileDatabase.findUserById(task.assignedBy);
        task.assignee = assignee ? { _id: assignee._id, name: assignee.name, email: assignee.email, department: assignee.department } : null;
        task.assignedBy = assignedBy ? { _id: assignedBy._id, name: assignedBy.name, email: assignedBy.email } : null;
      }
    } else {
      let query = {};
...
router.post('/', authenticateToken, requireManager, [
  ...
  body('assignee').notEmpty().withMessage('Valid assignee ID is required'),
  ...
  try {
    ...
    let assigneeUser, task;
    
    if (usingFileDatabase()) {
      // Check if assignee exists and is an employee
      assigneeUser = await fileDatabase.findUserById(assignee);
      if (!assigneeUser) {
      ...
      task = await fileDatabase.createTask({
        title,
        description,
        assignee,
      ...
      // Populate manually
      const assignedBy = await fileDatabase.findUserById(req.user._id);
      task.assignee = { _id: assigneeUser._id, name: assigneeUser.name, email: assigneeUser.email, department: assigneeUser.department };
      task.assignedBy = { _id: assignedBy._id, name: assignedBy.name, email: assignedBy.email };
    } else {
      // Check if assignee exists and is an employee
      ...
      task = await Task.findById(newTask._id)
        .populate('assignee', 'name email department')
        .populate('assignedBy', 'name email');
    }
...
router.get('/stats/overview', authenticateToken, requireManager, async (req, res) => {
  try {
    ...
    if (usingFileDatabase()) {
      stats = await fileDatabase.getTaskStats();
    } else {
